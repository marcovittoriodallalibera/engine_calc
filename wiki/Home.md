# Phase 360

Phase 360 is a visual calculation workbench for Vespa and other piston-controlled two-stroke engines. It combines port timing, rotary-valve timing, overlap diagnostics, compression and squish context, engine-character guidance, project comparison, print output and configurable transmission analysis.

## Latest verified desktop preview

- Current version: **0.1.0 preview 2**
- Published: **6 August 2026**

[Download Phase 360 for Windows or macOS](Downloads)

> The desktop packages are verified previews, not trusted public releases. Windows packages are unsigned. macOS packages are ad-hoc signed and not notarised. Read [Installation and security](Installation-and-security) before opening a download.

## Documentation

- [Downloads](Downloads)
- [Installation and security](Installation-and-security)
- [Calculation methodology](https://github.com/marcovittoriodallalibera/engine_calc/blob/main/docs/methodology.md)
- [Project format](https://github.com/marcovittoriodallalibera/engine_calc/blob/main/docs/project-format.md)
- [Security audit](https://github.com/marcovittoriodallalibera/engine_calc/blob/main/docs/security-audit.md)
- [Source repository](https://github.com/marcovittoriodallalibera/engine_calc)

Project files remain local unless you explicitly export or share them. Export important projects as JSON before replacing or removing the application.
