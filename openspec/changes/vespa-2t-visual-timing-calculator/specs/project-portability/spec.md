...
- **THEN** schema version 5 preserves the desired opening and closing angles without inventing a diameter, component split, or physical validity claim
...
- **THEN** schema version 5 preserves those selections while regenerating profile diagnostics, area and time-area series, uncertainty envelopes, and character annotations after restoration
...
- **THEN** the reader applies the same deterministic component migration while preserving its report identity, selecting profile `none`, preserving a recognised prior effective-area input as `constant-area` when present, inventing no uncertainty, and reconstructing schema version 5
...
- **THEN** the reader preserves the version 4 rotary authority, report identity, and recognised constant-area input, selects profile `none`, uses the documented default character RPM range, invents no physical measurement or uncertainty, and reconstructs schema version 5
...
- **THEN** the application restores its desired angles in timing-only mode with empty report identity and writes subsequent valid saves using the current storage key
...
- **THEN** the application applies the documented rotary migration, adds empty report identity, and writes a valid migrated or subsequently completed project using the schema version 5 storage key
...
- **THEN** the application preserves report identity, applies the documented rotary migration, and writes a valid migrated or subsequently completed project using the schema version 5 storage key
...
- **THEN** the application preserves its desired timing, manual-component authority, report identity, and recognised constant-area input, adds profile `none` without uncertainty, and writes subsequent valid saves using the schema version 5 storage key
...
- **WHEN** a valid schema-version-5 project contains a displayed complementary arc or converted component degrees
...
- **WHEN** an otherwise supported document contains unrecognised properties or persisted derived rotary edges, durations, overlaps, or converted angles
...
- **WHEN** a structurally valid schema-version-5 project names a recognised diagnostic profile but a built-in reference-set version unavailable in the current application
...
- **WHEN** a structurally valid version 1, 2, 3, or 4 document preserves desired timing but cannot establish physical rotary geometry without invention
...
Every portable representation that includes a diagnostic, geometric area curve, or Engine character estimate SHALL include the applicable claim level, selected profile and reference-set version, source and applicability, uncertainty status, and model boundary. Canonical JSON SHALL persist only the authoritative profile and input selections and SHALL regenerate diagnostics and series after validation rather than storing them as evidence.
